...
void main() {
    // 1. Dither Pattern (Pinned to Quad local UV)
    // Scale cells to be visible in UV space [0,1]
    vec2 cellSize = vec2(CELL_W, CELL_H) / vec2(800.0, 600.0); 
    vec2 cell = floor(v_texCoord / cellSize);
    vec2 cellOrigin = cell * cellSize;
    vec2 localFrac = (v_texCoord - cellOrigin) / cellSize;

    int qx = int(localFrac.x * 3.0);
    int qy = int(localFrac.y * 2.0);

    // 2. Video Sampling (Screen Space)
    vec3 cellCol = texture(u_video, v_videoCoord).rgb;
    ...
    float val = shapeResponse(cellCol[role]);

    // Sub-cell thresholding (3x2 grid)
    vec2 inSub = localFrac * vec2(3.0, 2.0);
    int subIdx = int(inSub.x) + int(inSub.y) * 3;

    float threshold;
    if      (subIdx == 0) threshold = 0.1;
    else if (subIdx == 1) threshold = 0.5;
    else if (subIdx == 2) threshold = 0.3;
    else if (subIdx == 3) threshold = 0.8;
    else if (subIdx == 4) threshold = 0.2;
    else                  threshold = 0.6;
...
export class DitherFilter extends FilterBase {
  name = 'Dither';
  isActive = true;
  ...
  render(ctx: FilterRenderContext) {
    ...
    if (!this.programInfo || !this.bufferInfo || !this.textures) return;
